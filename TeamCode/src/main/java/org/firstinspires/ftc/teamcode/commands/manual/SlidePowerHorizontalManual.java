...
public class SlidePowerHorizontalManual extends CommandBase {

    private final HorizontalSlide horizontalSlide;
    private final Supplier<Double> doubleSupplier;
    public SlidePowerHorizontalManual(HorizontalSlide horizontalSlide, Supplier<Double> doubleSupplier) {
    ...
    public void execute() {
        ...
        if (Math.abs(position) > 0.2) {
            horizontalSlide.setPower(position);
        }
    }
}
